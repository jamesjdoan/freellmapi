...
| Catalogue-wide copy | **Keys → provider → key row → Models & account limits** | Select **Copy free catalogue** to copy a credential‑blind Markdown snapshot of the curated FreeLLMAPI catalogue. Choose **All providers and models** or **Active providers and models** (routing‑enabled models whose provider holds a usable key). Excludes custom relay endpoints because their free‑tier status is unverified. Credentials, labels and internal IDs are never included.
