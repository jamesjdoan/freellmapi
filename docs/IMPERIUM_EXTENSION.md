...
| Provider/model limits and free-tier guidance | **Keys → provider → key row → Models & account limits** | Edit shared provider/model RPM, RPD, TPM and TPD beside each model. Use the right-hand panel for sourced allowances, scope, freshness and advisory flags. Verified current guidance can populate supported fields after confirmation. |
