...
| Catalogue-wide copy | **Keys → Providers tab header** | Select **Copy free catalogue** — a page-level action, because it spans every provider rather than the key row it used to sit under. Choose **All providers and models** or **Active providers and models** (routing-enabled models whose provider holds a usable key). Custom relay endpoints are excluded because their free-tier status is unverified. Credentials, labels and internal IDs are never included. |
