...
describe('per-attempt traces in the analytics requests API', () => {
  ...
  it('returns the ordered attempts ladder — with per-hop error summaries — in the per-request detail payload', async () => {
    ...
    expect(body.attempts).toEqual([
      { ordinal: 0, platform: 'groq', modelId: 'llama-x', keyOrdinal: 1, keyLabel: null, outcome: 'rate_limited', routing: null, startOffsetMs: 0, durationMs: 210, errorSummary: '429 rate limit reached, retry in 7m12s' },
      { ordinal: 1, platform: 'google', modelId: 'gemini-y', keyOrdinal: 2, keyLabel: null, outcome: 'timeout', routing: null, startOffsetMs: 215, durationMs: 10000, errorSummary: 'upstream timeout after 10000ms' },
      { ordinal: 2, platform: 'cerebras', modelId: 'qwen-z', keyOrdinal: 3, keyLabel: null, outcome: 'ok', routing: null, startOffsetMs: 10220, durationMs: 900, errorSummary: null },
    ]);
  });
