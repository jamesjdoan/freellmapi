import type { QuotaOutlookPool, QuotaOutlookResponse, Platform } from '@freellmapi/shared/types.js';
import { getQuotaStateForKeys, legacyPoolKey, type QuotaObservationView } from './provider-quota.js';
...
export function getQuotaOutlook(): QuotaOutlookResponse {
  ...
  for (const row of activity) {
    counts.set(row.pool, (counts.get(row.pool) ?? 0) + row.count);
    // Traffic is counted under the CURRENT pool identity, while a state row may
    // still carry the account-wide name recorded before the provider was
    // measured to meter per model (groq::account vs groq::model::<id>). Credit
    // both spellings, or a real reading shows zero traffic while its own
    // traffic reappears as a second, limitless phantom pool for one allowance.
    const legacy = legacyPoolKey(row.platform as Platform, row.pool);
    if (legacy) counts.set(legacy, (counts.get(legacy) ?? 0) + row.count);
  }
  ...
  // Traffic is counted under the CURRENT pool identity, while a state row may
  // still carry the account-wide name an install recorded before a provider was
  // measured to meter per model (groq::account vs groq::model::<id>). Without
  // bridging the two, a real reading shows zero traffic AND its own traffic
  // reappears as a second, limitless phantom pool for the same allowance.
  // `legacyPoolKey` is the same read-compat path the forecast already uses.
  // Pools a measured state row already speaks for, so the activity-only pass
  // below does not re-add the same allowance under its other spelling.
  const represented = new Set<string>();
  for (const row of selected.values()) represented.add(`${row.platform}\u0000${row.quotaPoolKey}`);
  ...
  for (const row of activity) {
    // `represented` covers both spellings, so traffic whose measured row is
    // filed under the old account-wide name is not added a second time here.
    const legacyOfActivity = legacyPoolKey(row.platform as Platform, row.pool);
    if (represented.has(`${row.platform}\u0000${row.pool}`)
        || (legacyOfActivity && represented.has(`${row.platform}\u0000${legacyOfActivity}`))) continue;
