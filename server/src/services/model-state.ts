...
export function clearCatalogModelTombstone(
  ...
  if (kind === 'chat' && removed.changes > 0) {
    ...
    //
    // `relist-restores-membership` off: reinstate the way upstream does, with
    // no chain rows. The membership stays recorded on the tombstone, so the
    // next relist restores it if the extension is back on by then.
    if (isExtensionEnabled('relist-restores-membership')) {
