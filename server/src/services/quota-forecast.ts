...
import { resolveQuotaPolicy, consumesPaidBalance, legacyPoolKey, inferQuotaPoolKey } from './provider-quota.js';
...
export function getRecentQuotaActivity(now: number): RecentQuotaActivity[] {
  ...
  return rows.map(row => ({ platform: row.platform, keyId: row.keyId,
    pool: inferQuotaPoolKey(row.platform, row.modelId), count: Number(row.count) }));
}
...
function entryFor(row: QuotaObservationView, ratePerMin: number | null, now: number): QuotaForecastEntry | null {
  ...
  if (remaining !== null) {
    ...
    // `quota-forecast-warnings` off: remaining/limit still report, but the
    // pool stops being flagged as low. No observation is discarded.
    lowBalance = isExtensionEnabled('quota-forecast-warnings')
      && ((absoluteApplies && remaining <= LOW_BALANCE_ABSOLUTE)
...
export function getQuotaForecast(
  ...
  for (const row of activity) {
    ...
    // Activity is keyed by the CURRENT pool identity; a state row may still be
    // filed under the account-wide name used before the provider was measured
    // to meter per model. Credit both spellings so a real reading is not shown
    // with zero traffic (same read-compat path as the supersede logic below).
    const legacy = legacyPoolKey(row.platform as Platform, row.pool);
    if (legacy) {
      const legacyIdentity = JSON.stringify([row.platform, legacy, row.keyId]);
      counts.set(legacyIdentity, (counts.get(legacyIdentity) ?? 0) + row.count);
    }
  }
