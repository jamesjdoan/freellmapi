...
export const IMPERIUM_EXTENSIONS: readonly ImperiumExtension[] = [
  ...
  },
  {
    ...
    summary: 'Mark a model that is free with no meter (OpenRouter stealth models such as Space Bunny) as unlimited: it skips every local usage limit, is counted toward none, is tried first in the chains it is in, and passes the paid-balance guard. On OpenRouter, AnyAPI and UnoRouter it only takes effect while the provider prices it at $0, re-checked at boot and every six hours; the moment it charges, every limit and the guard apply again. Cooldowns, health, capability and per-key concurrency always apply.',
    settingsLocation: 'Keys → expand a provider → the ∞ toggle on a model row',
    destinations: [{ kind: 'internal', label: 'Open provider keys', href: '/keys' }],
    category: 'routing',
    defaultEnabled: true,
    offBehaviour: 'Flagged models are treated like any other: every limit counts them and the paid-balance guard applies. Flags and price checks are kept.',
    takesEffect: 'Next routed request.',
    codeLocations: [
      'server/src/services/unlimited-models.ts',
      'server/src/services/router.ts (isUnlimitedModel gates, unlimitedFirst ordering, paid-guard filter)',
      'server/src/services/ratelimit.ts (recordRequest / recordTokens)',
      'server/src/lib/request-log.ts (requests.unlimited)',
      'server/src/db/migrations/20260925_000003_unlimited_models.ts',
