...
export class BlazeProvider extends OpenAICompatProvider {
  override async validateKey(apiKey: string, quotaContext?: QuotaObservationContext): Promise<KeyValidationResult> {
    const res = await this.fetchCatalogEndpoint('https://api.blazeapi.org/paid/v1/usage', apiKey, quotaContext);
    if (res.status === 401 || res.status === 403) return this.validationResult(res);
    if (res.ok || res.status === 402 || res.status === 429) return { valid: true };
    throw providerHttpError(res, `${this.name} key validation inconclusive`);
  }
