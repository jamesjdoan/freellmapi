...
async function runFallbackLoopAttempts(hooks: FallbackHooks, trace: RequestTrace): Promise<void> {
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    try {
      ...
      //
      // BOTH sinks fire, because they answer different questions and neither
      // subsumes the other: the row is the durable record the dashboard reads
      // afterwards, and upstream's line is what an operator tailing logs sees
      // at the moment of refusal, naming surface, request id and candidate
      // count. Keeping only one of them was a silent loss either way.
      if (!lastError) {
        logRoutingRefusal(hooks, routeErr, exhaustion, startedAt);
        logRoutingExhaustion(routeErr, hooks.logIdentity);
      }
